...
namespace Aikido.DAO
{
    ...
    {
        public List<dgvFee_ViewModel> filter(string nameClass)
        {
            ...
            using (var dataContext = new AccessDB_DAO())
            {
                ...
                var learnN = dataContext.Learns.Where(s => s.Class.Class_Name.Contains(nameClass) && s.RegisterDay.Year == monthht -1);
                var learn = dataContext.Learns.Where(s => s.Class.Class_Name.Contains(nameClass) && s.RegisterDay.Year == monthht);
                var learnP = dataContext.Learns.Where(s => s.Class.Class_Name.Contains(nameClass) && s.RegisterDay.Year == monthht + 1);
